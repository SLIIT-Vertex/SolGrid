...
export const descriptions = {
  ...
  "mobile-env": "Generate Android configuration from root .env",
  "mobile-build": "Configure and build the debug Android APK",
  "mobile-install": "Configure, build and install on connected Android devices",
  "mobile-test": "Configure and run Android JVM unit tests",
...
export function initializeMobileEnv(config, directory = root) {
  const value = config.MOBILE_API_BASE_URL;
  let url;
  try { url = new URL(value); } catch { throw new Error('MOBILE_API_BASE_URL must be an HTTP(S) URL ending with /'); }
  if (!['http:', 'https:'].includes(url.protocol) || !value.endsWith('/') || /[\s"\\]/.test(value) || url.username || url.password || url.search || url.hash) {
    throw new Error('MOBILE_API_BASE_URL must be an HTTP(S) URL ending with /, without credentials, query or fragment');
  }
  // Java Properties escaping keeps configuration data out of shell commands.
  const escape = value => String(value).replace(/\\/g, '\\\\').replace(/\r/g, '\\r').replace(/\n/g, '\\n').replace(/\t/g, '\\t').replace(/ /g, '\\ ');
  const content = '# Generated by make mobile-env; edit root .env instead.\n' +
    `API_BASE_URL=${escape(value)}\nMAPS_API_KEY=${escape(config.MOBILE_MAPS_API_KEY ?? '')}\n`;
  const path = `${directory}/mobile-app/env.properties`;
  if (!existsSync(path) || readFileSync(path, 'utf8') !== content) writeFileSync(path, content, { mode: 0o600 });
  console.log('Android configuration updated from root .env.');
}

function execute(command, args, config, cwd = root) {
  return new Promise((resolve, reject) => {
    // Windows npm and Gradle wrappers require a shell. Only fixed commands use it;
    // secret values and all other arguments go directly to the executable.
    const windowsWrapper = process.platform === 'win32' && (command === 'npm' || command === './gradlew');
    const child = spawn(windowsWrapper ? (command === 'npm' ? 'npm.cmd' : 'gradlew.bat') : command, args, {
      cwd, env: config, stdio: 'inherit', shell: windowsWrapper,
    });
...
export async function runTask(task, config = configuration()) {
  ...
  switch (task) {
    ...
    case 'mobile-env': return initializeMobileEnv(config);
    case 'mobile-build':
    case 'mobile-install':
    case 'mobile-test':
    case 'mobile-clean':
      if (task !== 'mobile-clean') initializeMobileEnv(config);
      return execute('./gradlew', [task === 'mobile-build' ? ':app:assembleDebug' : task === 'mobile-install' ? ':app:installDebug' : task === 'mobile-test' ? ':app:testDebugUnitTest' : 'clean'], config, `${root}/mobile-app`);
